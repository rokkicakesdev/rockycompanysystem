    ...
    <div class="row">
      ...
      <div class="col-lg-5">
        <div class="card">
          ...
          <div class="card-body p-0 recruitment-jobs-scroll">
            ...
            <div class="p-3 border-bottom <?= ($selectedJobId===$post['id'])?'bg-light':'' ?> recruitment-job-row"
                 onclick="window.location='recruitment.php?job_id=<?= $post['id'] ?>'">
      ...
      <div class="col-lg-7">
        ...
        <div class="card">
          ...
          <!-- ── FIX: flex header with title on left, button group on right ── -->
          <div class="card-header d-flex justify-content-between align-items-center flex-wrap recruitment-card-header">
            <!-- Title (truncates gracefully on narrow screens) -->
            <div class="recruitment-header-title">
              <strong><?= htmlspecialchars($selectedJob['title']) ?></strong>
              <span class="text-muted ml-1">— <?= htmlspecialchars($selectedJob['department_name']) ?></span>
            </div>
            <!-- Button group — stays right-aligned, never wraps outside the header -->
            <div class="recruitment-header-actions">
              ...
              <?php if ($selectedJob['status'] === 'open'): ?>
                <a href="recruitment.php?toggle_job=<?= $selectedJob['id'] ?>&new_status=closed&job_id=<?= $selectedJob['id'] ?>"
                   class="btn btn-xs btn-secondary">
                  <i class="fas fa-times-circle mr-1"></i>Close Posting
                </a>
              <?php else: ?>
                <a href="recruitment.php?toggle_job=<?= $selectedJob['id'] ?>&new_status=open&job_id=<?= $selectedJob['id'] ?>"
        ...
        <div class="card">
          <div class="card-body text-center py-5 text-muted">
            <i class="fas fa-mouse-pointer fa-2x mb-2 d-block recruitment-no-selection-icon"></i>
    ...
    <div class="modal-content">
      <form method="POST">
        ...
        <div class="modal-header">
          <h5 class="modal-title"><i class="fas fa-briefcase mr-2 text-primary"></i>Create Job Posting</h5>
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-primary"><i class="fas fa-plus mr-1"></i>Create Posting</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <form method="POST">
        ...
        <div class="modal-header">
          <h5 class="modal-title"><i class="fas fa-user-plus mr-2 text-primary"></i>Add Applicant</h5>
          <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
        <div class="modal-body">
          ...
          <div class="form-row">
            <div class="col form-group"><label>Email</label><input type="email" name="email" class="form-control"></div>
            <div class="col form-group"><label>Phone</label><input type="text" name="phone" class="form-control"></div>
          </div>
          <div class="form-group">
            <label>Source</label>
...
<div class="modal fade" id="updateApplicantModal" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <form method="POST">        <input type="hidden" name="update_applicant" value="1">
        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token) ?>">
        <input type="hidden" name="applicant_id" id="updateAppId">
        <div class="modal-header">
          <h5 class="modal-title"><i class="fas fa-user-edit mr-2 text-warning"></i>Update Applicant: <span id="updateAppName"></span></h5>
          <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
        <div class="modal-body">
          <div class="form-group">
